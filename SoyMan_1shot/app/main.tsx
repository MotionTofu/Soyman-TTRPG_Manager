...
function App() {
  ...
  async function backup() {
    try {
      const c = activeRef.current; if (!c?.content) return;
      download({ format: 'soyman-1shot-backup', version: 1, character: { ...c, revision: revision.current }, catalog: c.catalogKey ? await getCatalog(c.catalogKey) : null }, `oneshot-${c.id}.json`);
    } catch (e) { setError((e as Error).message); }
  }
  async function readFile(file: File) { if (file.size > 80 * 1024 * 1024) throw Error('Файл превышает 80 МБ'); return JSON.parse(await file.text()); }
  return <DndRuntimeContext.Provider value={{ allowDiceRolls: false, campaignConnected: false }}>
    <header className="oneshot-header"><a href="/" onClick={e => { if (status !== 'Сохранено на устройстве' && active?.content) { e.preventDefault(); setError('Дождитесь сохранения или скачайте резервную копию перед выходом.'); } }}>OneShot SoyMan</a><span className="muted">{active?.name || 'Ваши персонажи'}</span><span role="status">{status}</span>{active?.content && <button onClick={() => void backup()}>Скачать резервную копию</button>}</header>
