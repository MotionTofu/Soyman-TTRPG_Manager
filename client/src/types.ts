...
export interface CompendiumEntry {
  ...
  favourite?: boolean;
  /** Боевые роли существа (макс 2) — приходят со списком раздела для плиток. */
  combat_roles?: string[];
  /** Паспортный DPR из первого dnd-статблока (средний урон за свой ход при всех
      попаданиях). null — статблока нет или атак не разобрано. Со списком раздела. */
  dpr?: number | null;
  /** DPR посчитан эвристикой неточно (показывается с «~»). */
  dpr_approx?: boolean;
  /** КЗ/хиты из первого dnd-статблока — подстановка под ручные data.ac/hp,
      если те пусты. Ручное поле, где оно заполнено, главнее. Со списком раздела. */
  statblock_ac?: string | null;
  statblock_hp?: string | null;
...
// Типы D&D 5.5 переехали в общий с сервером пакет: нормализация листа должна
// быть одна на обе стороны, а значит и тип — один. Реэкспорт оставлен, чтобы
// 213 файлов, импортирующих `types.ts`, не пришлось править.
export type {
  DndAbilityScores,
  DndActionTiming,
  DndManualAttack,
  ...
  DndCreatureChallenge,
  DndCreatureSpellFrequency,
  DndCreatureSpellSlotLevel,
  DndAttackRollType,
  DndCreatureSpell,
