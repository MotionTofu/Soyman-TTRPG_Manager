...
function buildTheme(id: string, name: string, mode: ThemeMode, cfg: ThemeCfg): Theme {
  ...
  //
  // Тона — по шкале Полотна (§5 design_revision.md): пастель, насыщенность
  // <45%, светлота 78–88%. hsl(210 40% 84%), hsl(145 35% 84%), hsl(350 40% 85%).
  // Синий, зелёный и розовый различимы между собой лучше, чем любая тройка
  // с двумя соседними тонами.
  //
  // Палитра фиксирована и одинакова во всех режимах — как у нод Полотна, где
  // плашка тоже пастельная в любой теме, а под неё подбираются чернила. Первая
  // попытка подмешивать тон в тёмную бумагу проверку не прошла: в «Соевой
  // аберрации» три заливки вышли #363842/#373c3e/#3e363d — на глаз три
  // одинаковых серых, то есть кодирование переставало кодировать.
  //
  // Раз плашка светлая всегда, чернила на ней тоже свои и тёмные: строка
  // берёт цвет из `--skill-source-ink`, иначе в тёмных режимах на пастели
  // оказался бы светлый `--sb-ink`.
