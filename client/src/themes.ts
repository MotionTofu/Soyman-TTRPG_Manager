...
//
// Светлым режимам — пастель по шкале Полотна (§5 design_revision.md):
// насыщенность <45%, светлота 78–88%. hsl(210 40% 84%), hsl(145 35% 84%),
// hsl(350 40% 85%). Синий, зелёный и розовый различимы между собой лучше
// любой тройки с двумя соседними тонами.
//
// Тёмным — те же три тона, но подмесом в бумагу от насыщенного семени, а не
// от пастели. Обе более простые попытки провалились на живой проверке:
// пастель во всех режимах владелец увидел в «Соевом покое» как слишком яркую
// подсветку (бумага там #092328, плашка светлотой 84% — почти вспышка), а
// подмес самой пастели в тёмную бумагу дал в «Соевой аберрации»
// #363842/#373c3e/#3e363d, то есть три одинаковых серых. Насыщенное семя
// держит цветность на низкой светлоте: в «Покое» выходят #184461/#18553e/
// #4f303b — на шаг светлее бумаги и явно разные между собой.
//
// Чернила на плашке тоже свои: в светлом режиме плашка светлая и чернила
// тёмные, в тёмном — наоборот, иначе `--sb-ink` ложится не на ту подложку.
function skillSourceVars(mode: ThemeMode, paper: string, ink: string): Record<string, string> {
  if (mode === "dark") {
    return {
      "--skill-source-class": mixHex(paper, "#3380cc", 0.35),
      "--skill-source-background": mixHex(paper, "#33b268", 0.35),
      "--skill-source-both": mixHex(paper, "#d1475e", 0.35),
      "--skill-source-ink": ink,
      "--skill-source-ink-muted": mixHex(ink, paper, 0.35),
    };
  }
  return {
    "--skill-source-class": "#c6d6e7",
    "--skill-source-background": "#c8e4d4",
    "--skill-source-both": "#e8c9cf",
    "--skill-source-ink": "#1a1a1a",
    "--skill-source-ink-muted": "#5a5a5a",
  };
}
