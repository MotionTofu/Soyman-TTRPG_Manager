...
const HOTKEYS: { combo: string; where: string; does: string }[] = [
  { combo: "/", where: "любое место (фокус на странице, не в поле ввода)", does: "поставить курсор в глобальный поиск" },
  ...
  { combo: "Ctrl + K", where: "любое текстовое поле", does: "вставить внешнюю ссылку (Enter в поле адреса — вставить)" },
  { combo: "Ctrl + S", where: "правка текстовой карточки или имени персонажа", does: "сохранить, не выходя из правки" },
  ...
  { combo: "Ctrl + Enter", where: "дневник персонажа (поле новой заметки)", does: "отправить заметку" },
  { combo: "Enter / Esc", where: "поиск по листу персонажа", does: "перейти к первому найденному / очистить поиск" },
  ...
  { combo: "Space", where: "любое место (кроме полей ввода)", does: "воспроизведение / пауза фоновой музыки" },
  { combo: "Shift + S", where: "любое место", does: "остановить музыку" },
  { combo: "Shift + M", where: "любое место", does: "включить / выключить звук" },
  { combo: "Shift + ← / →", where: "любое место", does: "предыдущий / следующий трек" },
  { combo: "Shift + R", where: "любое место", does: "режим повтора (выкл → плейлист → трек)" },
  { combo: "Shift + X", where: "любое место", does: "перемешать порядок треков" },
  { combo: "Shift + ?", where: "любое место (кроме полей ввода)", does: "показать / скрыть подсказку горячих клавиш плеера" },
  { combo: "Ctrl + Z", where: "Полотно приключений", does: "отменить последнее действие" },
  ...
  { combo: "1–9", where: "Пульт звука (отдельное окно)", does: "проиграть стингер-звук (набор: цифры 1–9 или буквы QWE / ASD / ZXC)" },
  { combo: "[ / Х", where: "любое место (кроме полей ввода)", does: "скрыть/показать левую докстанцию превью" },
  { combo: "] / Ъ", where: "любое место (кроме полей ввода)", does: "скрыть/показать правый поиск" },
  { combo: "Ctrl + \\", where: "любое место (кроме полей ввода)", does: "скрыть/показать обе боковые панели" },
  { combo: "↑ / ↓", where: "Пульт сессии — переключатель сцен", does: "выбрать следующую/предыдущую сцену (Дальше + Назад + На игру)" },
            ...
            <div className="about-sub__body">
              ...
              <ul>
                ...
                <li>
                  ...
                  КЗ, атаками и особыми способностями.
                </li>
              </ul>
              <p className="muted">
