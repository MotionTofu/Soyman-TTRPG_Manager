...
interface PlayerGroupTabsProps {
  activeTab: string | null;
  onTabChange: (tab: string | null) => void;
  onGroupsChanged: () => void;
}

export function PlayerGroupTabs({ activeTab, onTabChange, onGroupsChanged }: PlayerGroupTabsProps) {
  ...
  async function loadGroups() {
    try {
      const data = await api.get<PlayerGroup[]>("/player-groups");
      setGroups(data);
    } catch {
      // silent
    }
  ...
  async function handleCreate() {
    ...
    if (!name) return;
    try {
      await api.post("/player-groups", { name });
      setNewName("");
      setCreating(false);
      await loadGroups();
      onGroupsChanged();
  ...
  async function handleRename() {
    ...
    if (!name) return;
    try {
      await api.put(`/player-groups/${renaming.groupId}`, { name });
      setRenaming(null);
      await loadGroups();
      onGroupsChanged();
    } catch (e) {
  ...
  async function handleDelete(groupId: number) {
    try {
      await api.del(`/player-groups/${groupId}`);
  ...
  return (
    <div className="tabs" role="tablist" aria-label="Группы игроков">
      <button
        role="tab"
      ...
      {deleteConfirm !== null && (
        <div className="modal-backdrop" onClick={() => setDeleteConfirm(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p>Игроки не будут удалены — они останутся в разделе «Все игроки».</p>
