...
function hitDie(entry: CompendiumEntry): string {
  const v = (entry.data as Record<string, unknown>)?.hit_die;
  return v != null && String(v).trim() ? String(v) : "";
}

function groupClasses(list: CompendiumEntry[], grouping: ClassGrouping, dir: ClassSortDir = "asc"): [string, CompendiumEntry[]][] {
  ...
  for (const e of byName) {
    if (grouping === "alpha") push(monogramLetter(e.name), e);
      ...
      const cat = (e.data as Record<string, unknown>)?.hit_die ? `Кость ${String((e.data as Record<string, unknown>).hit_die)}` : UNSET_LABEL;
      push(cat, e);
    }
  }
  const keys = [...map.keys()];
...
export function ClassTileGrid({ entries, grouping, sortDir = "asc", sectionId, searchActive, onToggleFavourite }: Props) {
  ...
  const groups = useMemo(() => groupClasses(entries.filter((e) => !e.favourite), grouping, sortDir), [entries, grouping, sortDir]);
  return (
    <div className="stack" style={{ gap: 10 }}>
      {favourites.length > 0 && <ClassGroup label="Избранное" sectionId={sectionId} list={favourites} forceOpen={searchActive} onOpenModal={setModal} onToggleFavourite={onToggleFavourite} />}
      {groups.map(([label, list]) => <ClassGroup key={label} label={label} sectionId={sectionId} list={list} forceOpen={searchActive} onOpenModal={setModal} onToggleFavourite={onToggleFavourite} />)}
      {modal && (
        <Modal onClose={() => setModal(null)}>
          <div className="stack">
            <h3 style={{ margin: 0 }}>{modal.name}</h3>
            <div className="muted" style={{ fontSize: "var(--fs-meta)" }}>{hitDie(modal) ? `Кость хитов ${hitDie(modal)}` : "Класс"}</div>
...
function ClassGroup({ label, list, sectionId, forceOpen, onOpenModal, onToggleFavourite }: { label: string; list: CompendiumEntry[]; sectionId: number; forceOpen: boolean; onOpenModal: (e: CompendiumEntry) => void; onToggleFavourite: (entry: CompendiumEntry, favourite: boolean) => void }) {
  const key = `class-group-${sectionId}-${label}`;
  ...
  return (
    <details className="comp-category" open={forceOpen || open} onToggle={(e) => { if (forceOpen) return; remember((e.currentTarget as HTMLDetailsElement).open); }}>
      ...
      <div className="monster-grid">{list.map((e) => <ClassTile key={e.id} entry={e} onOpenModal={onOpenModal} onToggleFavourite={onToggleFavourite} />)}</div>
    </details>
  );
}

const ClassTile = memo(function ClassTile({ entry, onOpenModal, onToggleFavourite }: { entry: CompendiumEntry; onOpenModal: (e: CompendiumEntry) => void; onToggleFavourite: (entry: CompendiumEntry, favourite: boolean) => void }) {
  const favourite = !!entry.favourite;
  const hd = hitDie(entry);
  return (
