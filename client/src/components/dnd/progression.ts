...
const ROLE_BY_HEADER: [RegExp, ProgressionRole][] = [
  ...
  // Короткое «Кол-во ячеек» той же таблицы Колдуна: без этого правила
  // «Разобрать заново из текста» роняет колонку в дефолтный resource,
  // и Договор магии снова перестаёт считаться (.scratch/warlock/issues/01).
  [/^(кол-?во|количество)\s+ячеек/i, "pact_slots"],
  [/^уровень\s+ячеек/i, "pact_level"],
  // Число известных воззваний — показатель, а не тратный пул: без правила
  // падает в дефолтный resource и становится ложным пулом «Воззвания».
  [/^воззвани/i, "stat"],
  [/^уровень/i, "level"],
