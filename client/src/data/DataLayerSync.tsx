 ...
 *   задето, и здесь перечитывается ровно это. Сигнал без адресата (запись
 *   мимо слоя) перечитывает всё.
 * - Игроки с других устройств (сокет → RealtimeListener → событие окна): каждое
 *   событие переводится в то, что оно задевает.
 *
 * Свои слушатели этих событий у компонентов продолжают работать: здесь
 * события только читаются. Монтируется один раз в корне.
 */
