"""Round tube/bottle body generator -- the toolkit's proving ground.

Straight cylindrical wall -> conical shoulder taper -> straight neck, revolved around Z,
flat-capped top and bottom (CurveToMesh's Fill Caps handles that automatically once the
profile no longer needs a degenerate zero-radius point -- an earlier draft of the plan
assumed a "base pole" point was required; empirically it isn't).
"""
...
def build_node_group(name):
    ...
    b.add_socket('Body Diameter', 'INPUT', 'NodeSocketFloat', key='body_diameter',
                 default=40.0, min_value=1.0, subtype='DISTANCE')
    ...
    # Label band -> material_index 1 (0 = Body), based on height fraction of the straight wall.
    ...
    links.new(in_band.outputs['Value'], store_matidx.inputs['Value'])  # float->int implicit conversion, confirmed live
...
class PACKAGING_OT_add_round_tube(bpy.types.Operator):
    bl_idname = 'object.packaging_add_round_tube'
    bl_label = 'Round Tube'
    bl_description = 'Add a procedural round tube/bottle body (Geometry Nodes)'
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        b = build_node_group('PKG_RoundTube')

        mesh = bpy.data.meshes.new('RoundTube')
        obj = bpy.data.objects.new('RoundTube', mesh)
        context.collection.objects.link(obj)

        mod = obj.modifiers.new('Packaging', 'NODES')
        mod.node_group = b.tree

        obj[properties.PKG_SHAPE_TYPE_KEY] = 'round_tube'
...
def register():
    bpy.utils.register_class(PACKAGING_OT_add_round_tube)


def unregister():
    bpy.utils.unregister_class(PACKAGING_OT_add_round_tube)
