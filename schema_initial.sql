...
CREATE TABLE IF NOT EXISTS entity_relations (
  ...
  label TEXT NOT NULL DEFAULT '', -- short user-written name, e.g. "╨╗╤О╨▒╨╕╤В", "╨╜╨╡╨╜╨░╨▓╨╕╨┤╨╕╤В"
...
-- Сворачиваемые разделы мастерения (см. server/src/db/schema.sql).
...
CREATE TABLE IF NOT EXISTS setting_beings (
  ...
  avatar_image_path TEXT,
  thumbnail_image_path TEXT, -- wide crop shown in the ╨Э╨░╤Б╨╡╨╗╨╡╨╜╨╕╨╡ list, independent of the square avatar
  folder_path TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  archived_at TEXT
);
...
-- Multi-article notes for a being's ╨Ш╤Б╤В╨╛╤А╨╕╤П/╨Я╨╛╨▓╨╡╨┤╨╡╨╜╨╕╨╡/╨в╨╡╨║╤Г╤Й╨░╤П ╤Б╨╕╤В╤Г╨░╤Ж╨╕╤П,
-- mirroring community_chapters. ╨в╨╡╨║╤Г╤Й╨░╤П ╤Б╨╕╤В╤Г╨░╤Ж╨╕╤П additionally uses
...
-- custom calendar tab (right-click on a day) or from the "╨б╨╛╨▒╤Л╤В╨╕╤П" list.
...
-- several beings/communities) тАФ many-to-many, replacing the old single
...
-- ("╨Т╨░╨╢╨╜╤Л╨╡ ╨┤╨░╤В╤Л") тАФ surfaced on the setting's own calendar preview and on
...
-- One-off in-world calendar events owned by a setting ("╨е╤А╨╛╨╜╨╕╨║╨░ ╨╝╨╕╤А╨░"),
...
-- touching this source row (deliberately not linked back тАФ see settings.ts).
...
-- Large named year ranges ("╨н╨┐╨╛╤Е╨░") a GM can define to group the ╨е╤А╨╛╨╜╨╕╨║╨░
-- ╨╝╨╕╤А╨░ event list by ╨н╨┐╨╛╤Е╨░ > ╨б╤В╨╛╨╗╨╡╤В╨╕╨╡ > ╨Ф╨╡╤Б╤П╤В╨╕╨╗╨╡╤В╨╕╨╡ > ╨У╨╛╨┤╨░ > ╨б╨╛╨▒╤Л╤В╨╕╤П instead
-- of a flat chronological list. An era spans from its own start_year up to
-- (but not including) the next era's start_year, ordered by start_year тАФ no
...
-- Simple app-wide key/value store (e.g. the "╨У╨╗╨░╨▓╨╜╨░╤П" page background image).
...
-- parent_id to form hierarchy (Class тЖТ subclasses/features). `kind` drives the
...
-- Custom GM-defined subsections under a campaign's "╨Ф╨╗╤П ╨╕╨│╤А╨╛╨║╨╛╨▓" tab. Each
...
-- Visibility is granted per-player via player_visibility_grants тАФ creating
...
-- Per-player, per-campaign reveal grants backing the "╨Ф╨╗╤П ╨╕╨│╤А╨╛╨║╨╛╨▓" tabs on
...
-- content (campaign_player_section тАФ whole-subsection grant implicitly
...
-- for "╨Ш╤Б╤В╨╛╤А╨╕╤П"/╨е╤А╨╛╨╜╨╕╨║╨░ ╨╝╨╕╤А╨░). Setting content is granted per campaign since
...
-- Player-authored "╨Ш╤Б╤Б╨╗╨╡╨┤╨╛╨▓╨░╨╜╨╕╨╡ ╨╝╨╕╤А╨░" journal тАФ simple ╤Б╤Г╤Й╨╡╤Б╤В╨▓╨╛/╨╗╨╛╨║╨░╤Ж╨╕╤П/
-- ╨┐╤А╨╡╨┤╨╝╨╡╤В/╤Б╨╛╨▒╤Л╤В╨╕╨╡ entries a player creates about what their party has
-- learned in-world. Shared across the whole campaign party (not private per
-- player) тАФ player_id just records who wrote the entry, it's not an access
-- filter. extra_field holds the one kind-specific value: ╨╝╨╡╤Б╤В╨╛ ╨╛╨▒╨╕╤В╨░╨╜╨╕╤П for
-- being, ╨╛╨▒╨╕╤В╨░╤В╨╡╨╗╨╕ for location; unused (empty) for item/event.
...
-- GM-authored reminder/message shown on a player's ╨У╨╗╨░╨▓╨╜╨░╤П in player-app.
...
-- entirely GM-controlled (no per-player dismiss/ack) тАФ deleting the row
...
-- Initiative tracker rows for the "╨Я╤Г╨╗╤М╤В ╤Б╨╡╤Б╤Б╨╕╨╕" cockpit (replaces the
-- global "╨Ь╨╡╤И╨╛╨║" widget in the search panel while on a session's live
...
-- "Also present in this setting" tags for the global ╨а╨╡╤Б╤Г╤А╤Б╤Л library тАФ
...
-- hard link to that existing file instead of a second physical copy тАФ same
...
-- as the hardlink source for the next duplicate upload тАФ not a "the" owner.
...
-- deleteWithArchiveChoice) and chooses "archive" over "delete forever" тАФ
-- reviewable/restorable from the Archive page's "╨д╨░╨╣╨╗╤Л" tab.
