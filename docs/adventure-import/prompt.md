...
(«Гоблин-воин»), если это понятно из текста. `name_original` у бестиария особенно важен:
монстра, которого в системе ещё нет, импортёр по нему и заведёт в компендиуме — под именем
вида «Головорез [Thug]». Если книга даёт полный статблок, разбери его в `statblock`:
именно он станет карточкой монстра в системе, а не только в приключении.

**Сообщества** — фракции, гильдии, народы, культуры, семьи.

**Сокровищница** — предметы: и легендарные артефакты, и обычный лут. `rarity` заполняй
только у вещей правил системы — тех, у кого редкость указана в книге. Реквизиту
приключения (гроссбух, письмо, ключ от склада) редкость не выдумывай: по её отсутствию
импортёр и отличает одно от другого.
